...
export default function EditProjectPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploadedNewImages: UploadedImage[] = []
  ...
  const handleRemoveExistingImage = async (image: ProjectImage) => {
    ...
    try {
      ...
      // Update state
      setExistingImages(prev => prev.filter(img => img.image_url !== image.image_url))
    } catch (err) {
  ...
  const handleSetCover = (imageUrl: string, isNew: boolean) => {
    if (isNew) {
      ...
      setNewImages(prev => prev.map(img => ({ ...img, isCover: img.url === imageUrl })))
    } else {
      // Update new images
      setExistingImages(prev => prev.map(img => ({ ...img, is_cover: img.image_url === imageUrl })))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasCover = existingImages.some(img => img.is_cover) || newImages.some(img => img.isCover)
    if (!hasCover) {
